...
Status (2026-09-25): bibliographic data is checked against web search results,
not against the papers themselves, because arXiv, Crossref, zbMATH and similar
hosts are blocked in the build environment. Once `arxiv.org` and
`export.arxiv.org` are reachable, `python3 tools/fetch_arxiv.py` downloads
every paper cited here as `arXiv:ID` and writes the arXiv metadata to
`literature/arxiv_metadata.json` (see §8).
...
- ✓ K. Voelkel, *Motivic cell structures for spherical varieties*,
  ...
  cell structures after finitely many $\mathbb{P}^1$-suspensions. It proves
  that **every spherical $k$-variety is stably cellular** in the sense of
  Dugger–Isaksen and that its Voevodsky motive is mixed Tate. This means the
  stable question in SPEC §3.5 is well posed for the whole target class.
...
- ✓ P. Brosnan, *On motivic decompositions arising from the method of
  Białynicki-Birula*, Invent. Math. 161 (2005), 91–111; arXiv:math/0407305.
...
- ✓ A. Weber, *Hirzebruch class and Białynicki-Birula decomposition*,
...
- ✓ M. Wendt, *Chow–Witt rings of Grassmannians*, Algebr. Geom. Topol. 24
  (2024); arXiv:1805.06142.
- ✓ T. Hudson, Á. Matszangosz, M. Wendt, *Chow–Witt rings and topology of flag
...
- ✓ S. Choi, H. Park, *On the cohomology and their torsion of real toric
...
- **?** *The cohomology rings of real toric spaces and smooth real toric
  varieties*, Proc. Roy. Soc. Edinburgh A; arXiv:2008.08961 (authors to
  check, probably Choi–Park or collaborators).
...
- ✓m F. Knop, *On the set of orbits for a Borel subgroup*, Comment. Math. Helv.
  70 (1995), 285–309.
- ✓ F. Knop, *Localization of spherical varieties*, arXiv:1303.2561.
- ✓ G. Gagliardi, *A combinatorial smoothness criterion for spherical
...
- ✓ G. Gagliardi, J. Hofscheier, H. Pearson, *Toricness and smoothness
  criteria for spherical varieties*, arXiv:2601.06376 (2026).
- ✓ G. Gagliardi, J. Hofscheier, H. Pearson, *The generalised Mukai conjecture
  for spherical varieties*, arXiv:2502.21155 (2025).
- ✓ K. Altmann, V. Kiritchenko, L. Petersen, *Merging divisorial with colored
  fans*, Michigan Math. J. 64 (2015), 3–38; arXiv:1210.4523.
- ✓ J. Tevelev, T. Vogiannou, *Spherical tropicalization*, arXiv:1511.02203.
...
- ✓m P. Bravi, G. Pezzini, *Primitive wonderful varieties*, Math. Z. (2016);
  arXiv:1106.3187.
- ✓m P. Bravi, G. Pezzini, *Wonderful varieties of type D*, Represent. Theory 9
  (2005); arXiv:math/0410472.
- **?** *Wonderful varieties of type B and C*, arXiv:0909.3771, and
  *Primitive spherical systems*, arXiv:0909.3765. Authors to check, probably
  Bravi, with or without Pezzini.
- ✓m S. Cupit-Foutou, *Wonderful varieties: a geometrical realization*,
  arXiv:0907.2852.
- ✓ R. Avdeev, *On computing the spherical roots for a class of spherical
...
- ✓ S. Banerjee, M. B. Can, *Equivariant K-theory of smooth projective
  spherical varieties*, arXiv:1603.04926.
- ✓ R. Gonzales, *Algebraic rational cells and equivariant intersection
...
- ✓m B. Pasquier, *Variétés horosphériques de Fano*, Bull. Soc. Math. France
  136 (2008), 195–225.
- ✓ S. Monahan, *Horospherical stacks and stacky coloured fans*, Trans. AMS 378
  (2025), 1167–1214; arXiv:2305.01571.
- **?** Cohomology of toroidal horospherical varieties as
  $H^*(G/P) \otimes \text{Stanley–Reisner}$ modulo a regular sequence: seen in
  search snippets (context of arXiv:1203.0671 and arXiv:2006.12043); the
  precise reference is still open.
...
- ✓m M. Goresky, R. Kottwitz, R. MacPherson, *Equivariant cohomology, Koszul
...
The build environment's egress policy currently rejects `arxiv.org`,
`export.arxiv.org`, `api.crossref.org`, `api.openalex.org`, `zbmath.org` and
`api.semanticscholar.org` (HTTP 403 at the proxy). Ways around this that stay
within policy:

1. **Recommended.** Add `arxiv.org` and `export.arxiv.org` to the allowed
   domains in the cloud environment's network settings. Then run
   `python3 tools/fetch_arxiv.py`. PDFs and TeX sources go to
   `literature/cache/`, which is git-ignored: arXiv's licence lets arXiv
   distribute the papers, not us. Metadata goes to
   `literature/arxiv_metadata.json`, which is committed.
2. Run the same script on any machine with normal internet access and commit
   only `literature/arxiv_metadata.json`. The cache stays local.
3. Adding `api.crossref.org` and `zbmath.org` would also allow checking the
   journal versions (volumes, pages, DOIs) automatically; not needed yet.

`python3 tools/fetch_arxiv.py --list` prints every arXiv ID currently cited
in this file.
